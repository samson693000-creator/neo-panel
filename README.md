...
Чистая установка с нуля (удалит старую папку `~/neo-panel`, поставит сервис, консоль можно закрыть):

```
curl -fsSL https://raw.githubusercontent.com/samson693000-creator/neo-panel/main/setup.sh | bash
```

Скрипт сам поставит Git, Python, venv, Node.js, скачает проект, поднимет systemd-сервис.
В конце выведет ссылку на админку, логин и пароль. После этого PuTTY можно закрывать.
